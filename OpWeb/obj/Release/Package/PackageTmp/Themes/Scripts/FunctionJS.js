...
function ReplyConfig(url, parm) {
    showConfirmMsg('注：您确认要把选中信息标记为已读吗？', function (r) {
        if (r) {
            getAjax(url, parm, function (rs) {
                if (parseInt(rs) > 0) {
                    showTipsMsg("标记成功！", 2000, 4);
                    windowload();
                } else if (parseInt(rs) == 0) {
                    showTipsMsg("标记失败，0 行受影响！", 3000, 3);
                }
                else {
...
function delCreateMsg(url, parm) {
    showConfirmMsg('注：您确认要生成消息序列吗？', function (r) {
        if (r) {
            getAjax(url, parm, function (rs) {
                if (parseInt(rs) > 0) {
                    showTipsMsg("创建成功！", 2000, 4);
                    windowload();
                } else if (parseInt(rs) == 0) {
                    showTipsMsg("创建失败，请重试！", 3000, 3);
                }
                else {
                    showTipsMsg("<span style='color:red'>创建失败，请稍后重试！</span>", 4000, 5);
                }
            });
        }
    });
}
function SendConfig(url, parm) {
    showConfirmMsg('注：您确认要发送信息吗？如果发现信息有误，可以在【用户消息】中，点击【删除】来隐藏错误信息！', function (r) {
        if (r) {
            getAjax(url, parm, function (rs) {
                if (parseInt(rs) > 0) {
                    showTipsMsg("发送成功！本次发送" + rs + "条", 2000, 4);
                    windowload();
                } else if (parseInt(rs) == 0) {
                    showTipsMsg("发送失败，0 行受影响！", 3000, 3);
                }
                else {
                    showTipsMsg("<span style='color:red'>发送失败，请稍后重试！</span>", 4000, 5);
                }
            });
        }
    });
}
function SubmitConfig(url, parm) {
    showConfirmMsg('注：提交审核后，内容将不可修改！', function (r) {
        if (r) {
            getAjax(url, parm, function (rs) {
                if (parseInt(rs) > 0) {
                    showTipsMsg("提交成功！", 2000, 4);
                    windowload();
                } else if (parseInt(rs) == 0) {
                    showTipsMsg("提交失败，0 行受影响！", 3000, 3);
                }
                else {
                    showTipsMsg("<span style='color:red'>提交失败，请稍后重试！</span>", 4000, 5);
                }
            });
        }
    });
...
function AuditConfig(url, parm) {
    showConfirmMsg('注：此操作不可恢复！', function (r) {
        if (r) {
            getAjax(url, parm, function (rs) {
                if (parseInt(rs) > 0) {
                    showTipsMsg("审核状态更改成功！", 2000, 4);
                    windowload();
                } else if (parseInt(rs) == 0) {
                    showTipsMsg("审核状态更改失败！", 3000, 3);
                }
                else {
                    showTipsMsg("<span style='color:red'>审核状态更改失败，请稍后重试！</span>", 4000, 5);
                }
            });
        }
    });
}
function OverConfig(url, parm) {
    showConfirmMsg('注：你确定要终结此票？', function (r) {
        if (r) {
            getAjax(url, parm, function (rs) {
                if (parseInt(rs) > 0) {
                    showTipsMsg("状态更改成功！", 2000, 4);
                    windowload();
                } else if (parseInt(rs) == 0) {
                    showTipsMsg("状态更改失败！", 3000, 3);
                }
                else {
                    showTipsMsg("<span style='color:red'>状态更改失败，请稍后重试！</span>", 4000, 5);
                }
            });
        }
    });
...
function DeleteData(url, parm) {
    showConfirmMsg("此操作不可恢复，您确定要继续吗？", function (r) {
        if (r) {
            getAjax(url, parm, function (rs) {
                if (parseInt(rs) > 0) {
                    showTipsMsg("删除成功！", 2000, 4);
