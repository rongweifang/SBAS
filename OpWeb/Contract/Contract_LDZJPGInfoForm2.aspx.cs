...
{
    public partial class Contract_LDZJPGInfoForm2 : System.Web.UI.Page
    {
        ...
        private void InitData()
        {
            ...
            sb.Append("SELECT * FROM Pledge_Voucher WHERE UID=@UID");
        ...
        {
            Hashtable ht = DataFactory.SqlDataBase().GetHashtableById("Pledge_Voucher", "PVID", this.PVID);
        ...
        protected void btn_save_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(this.PVID))
            {
                StringBuilder sb = new StringBuilder();
                sb.Append("SELECT * FROM Pledge_Voucher WHERE UID=@UID");
                SqlParam[] para = new SqlParam[] { new SqlParam("@UID", UID) };
            ...
            bool IsOk = DataFactory.SqlDataBase().Submit_AddOrEdit("Pledge_Voucher", "PVID", this.PVID, ht);
            if (IsOk)
            {
                ClientScript.RegisterStartupScript(Page.GetType(), "", "<script language=javascript>layer.msg('保存成功！'); window.location.href = '/Contract/Contract_LDZJPGInfoForm2.aspx?UID=" + UID + "';</script>");
            }
            else
            {
