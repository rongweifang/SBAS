...
{
    public partial class Contract_LDZJPGInfoForm4 : System.Web.UI.Page
    {
        ...
        private void InitData()
        {
            ...
            sb.Append("SELECT * FROM Pledge_Deposit WHERE UID=@UID");
        ...
        {
            Hashtable ht = DataFactory.SqlDataBase().GetHashtableById("Pledge_Deposit", "PDID", this.PDID);
        ...
        protected void btn_save_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(this.PDID))
            {
                StringBuilder sb = new StringBuilder();
                sb.Append("SELECT * FROM Pledge_Deposit WHERE UID=@UID");
                SqlParam[] para = new SqlParam[] { new SqlParam("@UID", UID) };
            ...
            bool IsOk = DataFactory.SqlDataBase().Submit_AddOrEdit("Pledge_Deposit", "PDID", this.PDID, ht);
            if (IsOk)
            {
                ClientScript.RegisterStartupScript(Page.GetType(), "", "<script language=javascript>layer.msg('保存成功！'); window.location.href = '/Contract/Contract_LDZJPGInfoForm4.aspx?UID=" + UID + "';</script>");
            }
            else
            {
