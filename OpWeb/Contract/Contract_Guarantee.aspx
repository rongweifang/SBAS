<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Contract_Guarantee.aspx.cs" Inherits="OpWeb.Contract.Contract_Guarantee" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        function edit() {
            ...
            if (IsEditdata(key)) {
                var url = "/Contract/Contract_GuaranteeForm.aspx?UID=" + key;
                top.openDialog(url, 'Contract_GuaranteeForm', '合同管理 - 个人担保合同', 1000, 610, 50, 50);
            }
        }
        //打印预览
        function detail() {
            var key = CheckboxValue();
            if (IsEditdata(key)) {
                var url = "/Contract/Contract_PrintView.aspx?UID=" + key + "&documentType=Contract_Guarantee";
                window.open(url);
        ...
        function detailA3() {
            var key = CheckboxValue();
            if (IsEditdata(key)) {
                var url = "/Contract/Contract_PrintViewA3.aspx?UID=" + key + "&documentType=Contract_Guarantee";
                window.open(url);
