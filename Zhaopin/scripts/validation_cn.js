...
Validator.messageSource['zh-cn'] = [
	...
	['validate-chinese-yx','请输入中文名称，格式：清华大学,华北电力大学(北京)'],
	['validate-chinese-zy','请输入中文名称，格式：新闻学,法律硕士(非法学)'],
	['validate-phone','请输入正确的电话号码,如:010-88888888,当前长度为%s.'],
	['validate-mobile-phone','请输入正确的手机号码,当前长度为%s.'],
	['validate-zip','请输入有效的邮政编码.'],
	['validate-qq','请输入有效的QQ号码.'],
	['validate-specialchar','请输入不包含特殊字符的值.'],
	['validate-specialchartextarea','请输入合法的字符的值.'],
	['validate-specialcharfilter','请输入合法的字符的值.'],
	['specialchar-spekc','请输入不包含特殊字符的值.'],
	['validate-nonumber','请输入不包含数字的值.'],
	['validate-name','姓名格式不正确.']
]
...
Validation.addAllThese([
	...
	/*
	 * Usage: int-range-sgtz-$minValue-$maxValue--shengao
	 * Example: -10 to 20 = int-range--10-20
	 */
	['int-range-sgtz',function(v,elm,args,metadata) {
		return (parseInt(v) >= parseInt(args[0]) && parseInt(v) <= parseInt(args[1]))
	},{depends : ['validate-integer']}],
	/*
	 ...
	 * Example: <input id='email' class='validate-ajax-http://localhost:8080/validate-email.aspx'>
	 */
	...
	['validate-chinese',/^[\u2e80-\ufe4f]+$/],
	['validate-phone',/^((0[0-9]{3})?(0[12][0-9])?[-])?\d{6,8}$/],
	['validate-mobile-phone',/(^0?[1][3578][0-9]{9}$)/],
	['validate-zip',/^[0-9]\d{5}$/],
	['validate-qq',/^[1-9]\d{4,8}$/],
	['validate-specialchar',/^[\u2e80-\ufe4fA-Za-z0-9,，@\s?_:;，。\,‘’''!“”\-_：《》=\/\\、\*\.#\""\$？\+！￥%……·~]+$/],
	['validate-specialchartextarea',/^[\u2e80-\ufe4fA-Za-z0-9,，@\s?_:;，。\,‘’''!“”\-_：《》=\/\\、\*\.#\""\$？\+！￥%……·~]+$/],
	['validate-specialcharfilter',/^[\u2e80-\ufe4f,\.@,a-z,A-Z,\d\-_\s?&=:,，《》\/]+$/],
	['specialchar-spekc',/^[\u2e80-\ufe4fA-Za-z0-9,，《》]+$/],
	['validate-nonumber',/^[\u2e80-\ufe4fA-Za-z]+$/],
	['validate-chinese-yx',/^[\u2e80-\ufe4f]+[(]*[\u2e80-\ufe4f]+[)]*[\u2e80-\ufe4f]*$/],
	['validate-chinese-zy',/^[\u2e80-\ufe4f]+[(]*[\u2e80-\ufe4f]+[)]*[\u2e80-\ufe4f]*$/],
	['validate-name',/^[\u2e80-\ufe4fA-Za-z]+$/]
]);
