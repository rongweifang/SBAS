...
using DataBase.DataBase.Common;
using System;
...
namespace Busines.DAL
{
    public class RM_System_Dal : RM_System_IDAO
    {
        ...
        public DataTable GetButtonHtml(string UserId)
        {
            ...
            //DataTable dt_Menu = DataFactory.SqlDataBase().GetDataTableProc("", ht);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT  M.Menu_Id,M.Menu_Name ,M.Menu_Title ,M.Menu_Img ,M.IconClass,M.TARGET ,M.ParentId ,M.NavigateUrl ,M.SortCode\r\n                            FROM    ( SELECT    M.Menu_Name ,M.Menu_Title ,M.Menu_Img,M.IconClass ,M.TARGET ,M.ParentId ,M.Menu_Id ,M.NavigateUrl ,M.SortCode ,'角色权限' AS TheirTYPE\r\n                                      FROM      Base_SysMenu M\r\n                                                LEFT JOIN Base_RoleRight R_R ON R_R.Menu_Id = M.Menu_Id\r\n                                                LEFT JOIN Base_UserRole U_R ON U_R.Roles_ID = R_R.Roles_ID\r\n                                      WHERE     M.TARGET = 'Iframe' AND U_R.User_ID = @User_ID\r\n                                                AND M.DeleteMark = 1 \r\n                                      UNION ALL\r\n                                      SELECT    M.Menu_Name ,M.Menu_Title ,M.Menu_Img,M.IconClass ,M.TARGET ,M.ParentId ,M.Menu_Id ,M.NavigateUrl ,M.SortCode ,'用户组权限' AS TheirTYPE\r\n                                      FROM      Base_SysMenu M\r\n                                                LEFT JOIN Base_UserGroupRight U_R ON U_R.Menu_Id = M.Menu_Id\r\n                                                LEFT JOIN Base_UserInfoUserGroup U_G ON U_G.UserGroup_ID = U_R.UserGroup_ID\r\n                                      WHERE     M.TARGET = 'Iframe' AND U_G.User_ID = @User_ID\r\n                                                AND M.DeleteMark = 1 \r\n                                      UNION ALL\r\n                                      SELECT    M.Menu_Name ,M.Menu_Title ,M.Menu_Img ,M.IconClass,M.TARGET ,M.ParentId ,M.Menu_Id ,M.NavigateUrl ,M.SortCode ,'用户权限' AS TheirTYPE\r\n                                      FROM      Base_SysMenu M\r\n                                                LEFT JOIN Base_UserRight U_R ON U_R.Menu_Id = M.Menu_Id\r\n                                      WHERE     M.TARGET = 'Iframe' AND U_R.User_ID = @User_ID\r\n                                                AND M.DeleteMark = 1\r\n                                    ) M\r\n                            GROUP BY M.Menu_Id ,M.Menu_Name ,M.Menu_Title ,M.Menu_Img,M.IconClass ,M.TARGET ,M.ParentId ,M.NavigateUrl ,M.SortCode ORDER BY M.SortCode");
            SqlParam[] para = new SqlParam[]
			{
				new SqlParam("@User_ID", UserId)
			};
        ...
        {
            bool result;
            LogHelper Logger = new LogHelper("文件帮助类");
            try
            {
                string[] Connection = ConfigHelper.GetAppSettings("SqlServer_RM_DB").Split(new char[]
				{
